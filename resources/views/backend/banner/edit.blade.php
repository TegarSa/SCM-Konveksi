                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('banner.update', $banner->id) }}" method="POST"
                            ...
                            @csrf
                            <div class="mb-3 row">
                                <label class="col-form-label col-sm-2 text-sm-end">Status</label>
                                <div class="col-sm-10">
                                    <select class="form-select  @error('active') is-invalid @enderror" name="active">
                                        @if ($banner->active == 0)
                                            <option value="{{ $banner->active }}" selected>Show</option>
                                        @else
                                            <option value="{{ $banner->active }}" selected>Hidden</option>
                                        @endif
